...
def send(update, message, keyboard, backup_message):
    try:
        ...
        msg = update.effective_message.reply_text(markdown_parser(backup_message +
                                                                  "\nNote: el mensaje actual es "
                                                                  "problemas con markdown. Puede ser "
        ...
        msg = update.effective_message.reply_text(markdown_parser(backup_message +
                                                                  "\nNote: El mensaje actual es "
                                                                  "Invalido"
                                                                  "curly brackets. Actualizar"),
                                                  parse_mode=ParseMode.MARKDOWN)
    except BadRequest as excp:
        if excp.message == "Button_url_invalid":
            msg = update.effective_message.reply_text(markdown_parser(backup_message +
                                                                      "\nNote: Esta mal la url "
                                                                      "en uno de estos botones."),
                                                      parse_mode=ParseMode.MARKDOWN)
        elif excp.message == "Protocolo url no soportado":
            msg = update.effective_message.reply_text(markdown_parser(backup_message +
                                                                      "\nNote: Estos botones no son soportados por T"),
                                                      parse_mode=ParseMode.MARKDOWN)
        elif excp.message == "Wrong url host":
            msg = update.effective_message.reply_text(markdown_parser(backup_message +
                                                                      "\nNote: Estan masl las url."),
                                                      parse_mode=ParseMode.MARKDOWN)
...
def del_joined(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if not args:
        ...
        if del_pref:
            update.effective_message.reply_text("Mensaje de entrada deria estarse borrando.")
        else:
            update.effective_message.reply_text("No estoy borrando entradas")
        return ""

    if args[0].lower() in ("on", "yes"):
        sql.set_del_joined(str(chat.id), True)
        update.effective_message.reply_text("Tratare de borrar mensajes viejos!")
        return "<b>{}:</b>" \
        ...
        update.effective_message.reply_text("No voya borrar mensajes viejos.")
        return "<b>{}:</b>" \
               "\n#CLEAN_SERVICE_MESSAGE" \
               "\n<b>Admin:</b> {}" \
               "\nCambiaste mensajes de union a <code>OFF</code>.".format(html.escape(chat.title),
                                                                          mention_html(user.id, user.first_name))
    else:
        # idek what you're writing, say yes or no
        update.effective_message.reply_text("Entiendo solo 'on/yes' o 'off/no' nada mas!")
        return ""
    ...
    if should_welc:
        ...
        for new_mem in new_members:
            ...
            if new_mem.id == OWNER_ID:
                update.effective_message.reply_text("Llego el maestro!")
                continue
    ...
    if should_goodbye:
        ...
        if left_mem:
            ...
            if left_mem.id == OWNER_ID:
                update.effective_message.reply_text("RIP Maestro")
                return
...
def welcome(bot: Bot, update: Update, args: List[str]):
    ...
    if len(args) == 0 or args[0].lower() == "noformat":
        ...
        if args[0].lower() in ("on", "yes"):
            ...
            update.effective_message.reply_text("Ya no saludo.")

        else:
            # idek what you're writing, say yes or no
            update.effective_message.reply_text("Solo entiendo 'on/yes' o 'off/no' solamente!")
...
def goodbye(bot: Bot, update: Update, args: List[str]):
    ...
    if len(args) == 0 or args[0] == "noformat":
        ...
        if args[0].lower() in ("on", "yes"):
            ...
            update.effective_message.reply_text("Se fue, que se joda!")

        elif args[0].lower() in ("off", "no"):
            sql.set_gdbye_preference(str(chat.id), False)
            update.effective_message.reply_text("Se fue? me vale.")

        else:
            # idek what you're writing, say yes or no
            update.effective_message.reply_text("Entiendo 'on/yes' o 'off/no' nada mas!")
...
def set_welcome(bot: Bot, update: Update) -> str:
    ...
    if data_type is None:
        msg.reply_text("No dijiste con que responder!")
        return ""

    sql.set_custom_welcome(chat.id, content or text, data_type, buttons)
    msg.reply_text("Bienvenida lista!")
...
def reset_welcome(bot: Bot, update: Update) -> str:
    ...
    update.effective_message.reply_text("Bienvenida por defecto!")
    return "<b>{}:</b>" \
           "\n#RESET_WELCOME" \
           "\n<b>Admin:</b> {}" \
